# -*- coding: utf-8 -*-
"""08_XGBoost.py  -  XGBoost (6 independent XGBs per T)  +/-flow x PUB/PUR
XGBoost uses fixed random_state per run; multi-seed is supported for robustness.
"""
...
def resolve_seeds() -> list[int]:
    ...
    raw = args.seeds if args.seeds is not None else os.environ.get("XGB_SEEDS", "")
    if str(raw).strip() == "":
    ...
    try:
        ...
        logging.warning("Invalid --seeds/XGB_SEEDS=%r, fallback to default %s", raw, DEFAULT_SEEDS)
        return DEFAULT_SEEDS.copy()
...
if __name__ == "__main__":
    import torch
    seeds = resolve_seeds()
    logging.info("XGBoost run seeds: %s", seeds)
    if len(seeds) == 1:
